...
const BUILT_IN_THEMES = {
    puppy: {
        ...
        colors: {
            ...
            '--cream': '#fffaf3',
            '--shadow': '0 4px 18px rgba(255, 144, 184, 0.18)',
            '--shadow-soft': '0 2px 10px rgba(255, 144, 184, 0.12)',
            '--nav-bg': '#fff5f9'
        },
        stickers: 'assets/stickers/',
        bgPattern: "url(\"data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='60' height='60' viewBox='0 0 60 60'><path d='M30 18c-2-4-8-4-8 0 0 4 8 8 8 8s8-4 8-8c0-4-6-4-8 0z' fill='%23ffb8d4' opacity='0.22'/></svg>\")"
    },
    hellokitty: {
        name: 'Hello Kitty',
        editable: false,
        colors: {
            ...
            '--cream': '#fff5f7',
            '--shadow': '0 4px 18px rgba(255, 107, 138, 0.18)',
            '--shadow-soft': '0 2px 10px rgba(255, 107, 138, 0.12)',
            '--nav-bg': '#fff0f3'
        },
        stickers: 'assets/stickers-kitty/',
        bgPattern: "url(\"data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='60' height='60' viewBox='0 0 60 60'><circle cx='30' cy='30' r='3' fill='%23ff6b8a' opacity='0.15'/><path d='M20 15 L25 8 L27 15 Z M33 15 L35 8 L40 15 Z' fill='%23ff6b8a' opacity='0.12'/></svg>\")"
    }
};

// 主题管理器
const ThemeManager = {
    STORAGE_KEY: 'shen_themes',
    ACTIVE_KEY: 'shen_active_theme',
    ...
    // 获取当前激活的主题ID
    getActiveThemeId() {
        return localStorage.getItem(this.ACTIVE_KEY) || 'puppy';
    },
    ...
    applyTheme(themeId) {
        ...
        if (theme.bgPattern) {
            document.body.style.backgroundImage = `
                radial-gradient(circle at 15% 20%, ${theme.colors['--primary']}14 0%, transparent 40%),
                radial-gradient(circle at 85% 80%, ${theme.colors['--primary-light']}80 0%, transparent 40%),
                ${theme.bgPattern}
            `;
            document.body.style.backgroundSize = 'auto, auto, 120px 120px';
        }

        // 更新贴纸路径（如果页面有贴纸元素）
        document.querySelectorAll('.sticker[data-theme-sticker]').forEach(img => {
            const stickerName = img.dataset.themeSticker;
            img.src = theme.stickers + stickerName;
        });

        // 保存当前主题
        localStorage.setItem(this.ACTIVE_KEY, themeId);
    ...
    saveCurrentAsTheme(name) {
        ...
        const colors = {};
        Object.keys(BUILT_IN_THEMES.puppy.colors).forEach(key => {
            colors[key] = computedStyle.getPropertyValue(key).trim() || BUILT_IN_THEMES.puppy.colors[key];
        });
        ...
        const theme = {
            ...
            colors: colors,
            stickers: 'assets/stickers/',
            bgPattern: BUILT_IN_THEMES.puppy.bgPattern
        };
    ...
    // 删除自定义主题
    deleteTheme(themeId) {
        if (BUILT_IN_THEMES[themeId]) return false; // 不能删内置主题
        const custom = JSON.parse(localStorage.getItem(this.STORAGE_KEY) || '{}');
        if (custom[themeId]) {
    ...
    updateThemeUI() {
        ...
        Object.entries(themes).forEach(([id, theme]) => {
            ...
            item.className = `theme-item ${id === activeId ? 'active' : ''}`;
            item.innerHTML = `
                <div class="theme-preview" style="background: ${theme.colors['--primary']}; border: 2px solid ${theme.colors['--primary-dark']}"></div>
                <div class="theme-info">
                    <span class="theme-name">${theme.name}</span>
                    ${id === activeId ? '<span class="theme-badge">当前</span>' : ''}
                </div>
                <div class="theme-actions">
                    ${id !== activeId ? `<button class="theme-btn apply" onclick="ThemeManager.applyTheme('${id}')">使用</button>` : ''}
                    ${theme.editable ? `
                        <button class="theme-btn rename" onclick="ThemeManager.promptRename('${id}')">改名</button>
                        <button class="theme-btn delete" onclick="ThemeManager.confirmDelete('${id}')">删除</button>
                    ` : ''}
                </div>
            `;
    ...
    // 弹窗重命名
    promptRename(themeId) {
        const themes = this.getAllThemes();
        const currentName = themes[themeId]?.name || '';
        const newName = prompt('给主题起个新名字：', currentName);
        if (newName && newName.trim()) {
            this.renameTheme(themeId, newName.trim());
        }
    },
    ...
    confirmDelete(themeId) {
        const themes = this.getAllThemes();
        const name = themes[themeId]?.name || '该主题';
    ...
    // 弹窗保存新主题
    promptSave() {
        const name = prompt('给新主题起个名字：', '');
        if (name && name.trim()) {
            const id = this.saveCurrentAsTheme(name.trim());
            this.applyTheme(id);
            alert(`主题「${name.trim()}」已保存！`);
        }
    },
