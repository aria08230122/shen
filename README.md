...
### 其他
- 💾 本地优先保存：断网或云端暂停时仍可正常记录
- ☁️ Supabase 待同步队列：恢复后自动补传，失败原因会明确显示
- 📱 PWA 支持，可添加到主屏幕
- 🔥 连续记账天数打卡
- 💬 每日一句（首页随机显示）
- 📤 完整 JSON 导入/导出（账目、奶茶、留言、待办、预算、分类和主题）
- 🎀 可爱粉紫色 UI
...
连接成功后，应用会先上传本机尚未同步的数据，再安全拉取云端数据。以后每次新增、编辑或删除都会先写入本机；云端不可用时，设置页会显示待同步数量，并在网络恢复、重新打开网页或回到页面时自动重试。也可以随时点「立即同步」。

### 3. 备份与恢复

设置 → 数据管理中可以：

- 导出完整 JSON 备份。备份不包含 Supabase Project URL 和 Anon Key。
- 导入本应用当前格式或旧版导出的 JSON。导入后会立即恢复本机数据；已配置 Supabase 时会继续同步到云端。
- 清空账目、奶茶、留言、待办、预算和自定义分类，并同步清理云端对应数据。
...
**多设备数据不一致？**
先在有离线记录的设备上打开网页，等设置页显示「已同步」，再在另一台设备点「立即同步」。本应用会先上传待同步操作，再拉取云端数据，不会用一次直接拉取覆盖尚未上传的本机记录。

**Supabase 又暂停或暂时连不上怎么办？**
可以继续正常记账，数据会立即保存在当前浏览器。设置页会显示等待同步的项目数；项目恢复后应用会自动重试，也可以手动点「立即同步」。
